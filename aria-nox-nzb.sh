...
aria2c --allow-overwrite=true --auto-file-renaming=true --bt-enable-lpd=true --bt-detach-seed-only=true \
       --bt-remove-unselected-file=true --bt-tracker="[$tracker_list]" --bt-max-peers=5 --enable-rpc=true \
       --max-connection-per-server=1 --max-concurrent-downloads=1 --split=1 \
       --seed-ratio=0 --check-integrity=true --continue=true --daemon=true --disk-cache=4M --force-save=true \
